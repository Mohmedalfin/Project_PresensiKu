...
$result = mysqli_query($connection, "SELECT presensi.*, pegawai.name, pegawai.lokasi_presensi, pegawai.nip  FROM presensi JOIN pegawai  ON presensi.id_pegawai = pegawai.id WHERE DATE_FORMAT(tanggal_masuk, '%Y-%m') = '$filter_tahun_bulan' ORDER BY tanggal_masuk DESC");
...
$sheet->setCellValue('C2', isset($_POST['filter_bulan']) ? $_POST['filter_bulan'] : '');
$sheet->setCellValue('C3', isset($_POST['filter_tahun']) ? $_POST['filter_tahun'] : '');
